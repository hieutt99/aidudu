...
class LabelViewSet(ModelViewSet):
    ...
    def create_label(self, request,):
        board_id = parse_int_or_400(request.data, 'board')
        board = Board.objects.filter(id=board_id)
        board_membership = BoardMembership.objects.filter(user_id=request.user, board=board)
        if not board_membership.exists():
            raise PermissionDenied(detail="You do not belong to this board or this board doesn't exist.")
        label = Label.objects.create(board_id=board_id, name=request.data.name, color=request.data.color)
        if 'card' in request.data:
            card_id = parse_int_or_400(request.data, 'card')
            card_label = CardLabelRelationship.objects.create(card_id=card_id, label_id=label.id)
        return Response(status=status.HTTP_204_NO_CONTENT)